...
def star_nfa(nfa1):
    n = nfa1.n
    a = len(nfa1.accept_states) + 1
    t = nfa1.t + nfa1.a * len(nfa1.transitions[0])
    accept_states = [0]
    for i in nfa1.accept_states:
        accept_states.append(i + 1)
    transitions = []
    ...
    for state in nfa1.accept_states:
        for k in nfa1.transitions[0]:
            transitions[state].append(Transition(k.symbol, k.state))
    return NFA(n, a, t, 0, accept_states, transitions)
